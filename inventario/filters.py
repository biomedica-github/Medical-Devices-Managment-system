...
class filtro_equipo(django_filters.FilterSet):
    numero_nacional_inv = django_filters.CharFilter(
        field_name="numero_nacional_inv",
        lookup_expr="icontains",
        label="Numero nacional de inventario",
    )
    nombre_equipo = django_filters.CharFilter(
        field_name="nombre_equipo", lookup_expr="icontains", label="Nombre del equipo"
    )
    marca = django_filters.CharFilter(
        field_name="marca", lookup_expr="icontains", label="Marca del equipo"
    )
    modelo = django_filters.CharFilter(
        field_name="modelo", lookup_expr="icontains", label="Modelo del equipo"
    )
...
class filtro_equipo_servicio(django_filters.FilterSet):
    mes_servicio = django_filters.ChoiceFilter(
        choices=fecha_choices,
        field_name="fecha",
        lookup_expr="month",
        label="Mes cuando se realizo servicio",
    )
    año_servicio = django_filters.NumberFilter(
        field_name="fecha", lookup_expr="year", label="Año en que se realizo servicio"
    )
    numero_orden = django_filters.CharFilter(
        field_name="numero_orden",
        lookup_expr="icontains",
...
class filtro_equipo_agenda(django_filters.FilterSet):
    mes = django_filters.ChoiceFilter(
        choices=fecha_choices, field_name="fecha", lookup_expr="month", label="Mes"
    )
    año = django_filters.NumberFilter(
        field_name="fecha", lookup_expr="year", label="Año"
    )
...
class filtro_agenda(django_filters.FilterSet):
    ...
    shortcut = django_filters.DateRangeFilter(
        field_name="fecha", label="Atajos de fecha"
    )
    tipo_evento = django_filters.ChoiceFilter(
        choices=TIPO_OPCIONES, field_name="tipo_evento", label="Tipo de evento"
    )

    class Meta:
        model = models.Evento
        fields = ["mes", "fecha", "año", "shortcut", "tipo_evento", "estado"]


class filtro_equipo_checklist(django_filters.FilterSet):
    mes = django_filters.ChoiceFilter(
        choices=fecha_choices, field_name="fecha_hora", lookup_expr="month", label="Mes"
    )
    dia = django_filters.NumberFilter(
        field_name="fecha_hora", lookup_expr="day", label="Dia"
    )
    año = django_filters.NumberFilter(
        field_name="fecha_hora", lookup_expr="year", label="Año"
    )
...
class filtro_contrato(django_filters.FilterSet):
    num_contrato = django_filters.CharFilter(
        field_name="num_contrato", lookup_expr="icontains", label="Numero de contrato"
    )
    año_vencimiento = django_filters.NumberFilter(
        field_name="fecha_vencimiento", lookup_expr="year", label="Año de vencimiento"
    )
    mes_vencimiento = django_filters.ChoiceFilter(
        choices=fecha_choices,
...
class filtro_ordenpendiente(django_filters.FilterSet):
    ...
    prueba = django_filters.BooleanFilter(
        field_name="fecha",
        method="filtro_vencido",
        label="Mostrar solo ordenes pasadas de fecha",
    )
...
class filtro_areas_general(django_filters.FilterSet):
    nombre_sala = django_filters.CharFilter(
        field_name="nombre_sala", lookup_expr="icontains", label="Nombre de la sala"
    )
...
class filtro_areas_equipo(django_filters.FilterSet):
    cama = django_filters.NumberFilter(
        field_name="cama",
        lookup_expr="exact",
        label="Cama donde se encuentra el equipo",
    )
    numero_nacional_inv = django_filters.CharFilter(
        field_name="numero_nacional_inv",
        lookup_expr="icontains",
