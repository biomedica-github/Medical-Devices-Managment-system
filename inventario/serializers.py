...
def calcular_fecha(datetimeobject: datetime.date) -> dict:
    return {
        "dia": datetimeobject.strftime("%A").capitalize(),
...
class ContratoProveedor(serializers.ModelSerializer):
    ...
    dias_restantes = serializers.SerializerMethodField(
        method_name="calcular_dias_restantes"
    )
...
class ContratoSerializers(serializers.ModelSerializer):
    ...
    tipo_contrato = serializers.SerializerMethodField(method_name="tipo_de_contrato")
    tipo_servicio_estipulado = serializers.SerializerMethodField(
        method_name="tipo_servicio"
    )
...
class ModificarContratoSerializer(serializers.ModelSerializer):
    ...
    tipo_servicio_estipulado = serializers.ChoiceField(
        choices=Contrato.SERVICIO_OPCIONES
    )
...
class CrearContratoSerializer(serializers.ModelSerializer):
    ...
    equipos_contrato = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=Equipo_medico.objects.filter(contrato=None),
        label="Equipos medicos sin contrato",
    )
...
class AgregarContratoProveedorSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        ...
        self.instance = Contrato.objects.create(
            proveedor=proveedor_objeto, **self.validated_data
        )
        return self.instance
...
class CrearOrdenSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "estatus",
            "responsable",
            "autorizo_jefe_biomedica",
            "autorizo_jefe_conservacion",
    ...
    id = serializers.IntegerField(read_only=True)
    orden_escaneada = serializers.FileField(
        validators=[FileExtensionValidator(allowed_extensions=["pdf"])], required=False
    )
...
class AgregarEventoEquipoSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        ...
        equipo_med = Equipo_medico.objects.get(id=equipo)
        self.instance = orden = Evento.objects.create(
            equipo_medico=equipo_med, **self.validated_data
        )
        return self.instance
...
class AgregarEventoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Evento
        fields = ["fecha", "equipo_medico", "tipo_evento", "estado"]


class modificarEventoVencimientoContrato(serializers.ModelSerializer):
    class Meta:
        model = Evento
        fields = ["fecha", "estado"]


class AgregarEventoPOSTSerializer(serializers.ModelSerializer):
    TIPO_SERVICIO = "SERVC"
    TIPO_CAPACITACION = "CAPAC"
    TIPO_CHECKLIST = "CHECK"
    TIPO_OPCIONES = [
...
class AgendaAdminSerializer(serializers.ModelSerializer):
    ...
    dias_restantes_num = serializers.SerializerMethodField(
        method_name="get_dias_faltan", read_only=True
    )
...
class OrdenAgendaAreaUsuarioVerSerializer(serializers.ModelSerializer):
    ...
    dias_restantes = serializers.SerializerMethodField(
        method_name="calcular_dias_restantes", read_only=True
    )
    dias_restantes_num = serializers.SerializerMethodField(
        method_name="get_dias_faltan", read_only=True
    )
    ...
    def save(self, **kwargs):
        ...
        if self.instance is not None:
            ...
            self.instance = orden = Orden_Servicio.objects.create(
                fecha=fecha_data, contrato=contrato_objeto
            )
...
class EquipoBajaSerializer(serializers.ModelSerializer):
    ...
    carta_obsolescencia_tercero = serializers.FileField(
        validators=[FileExtensionValidator(allowed_extensions=["pdf"])], allow_null=True
    )
    dictamen_tecnico_propio = serializers.FileField(
        validators=[FileExtensionValidator(allowed_extensions=["pdf"])], allow_null=True
    )
    minuta_baja = serializers.FileField(
        validators=[FileExtensionValidator(allowed_extensions=["pdf"])], allow_null=True
    )
...
class Equipo_Serializer(serializers.ModelSerializer):
    ...
    area_href = serializers.SerializerMethodField(
        method_name="get_area", read_only=True
    )
    contrato_href = serializers.SerializerMethodField(
        method_name="get_contrato", read_only=True
    )
...
class AgregarEquipoAreaSerializer(serializers.ModelSerializer):
    ...
    equipos_area = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=Equipo_medico.objects.filter(area=None),
        label="Equipos medicos sin area establecida",
    )
...
class CrearCheckListSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        ...
        CheckList.objects.create(
            area=sala_query, equipo=equipo_query, **self.validated_data
        )
        return self.instance
...
class CrearReporteSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        ...
        self.instance = ReporteUsuario.objects.create(
            area=sala_query,
            equipo=equipo_query,
            responsable=usuario,
            **self.validated_data,
...
class VerReportesSerializer(serializers.ModelSerializer):
    ...
    fecha_entrega_str = serializers.SerializerMethodField(
        method_name="get_fecha_entrega"
    )
